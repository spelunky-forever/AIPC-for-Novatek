...
| QNN-5 | Context Binary Generation | ✅ Done (skipped by config — see §5, Issue #6) |
| QNN-6 | Inference (aipc wrapper) | ✅ Done (acceptance PASS ×3) |
...
|---|---|---|
| Acceptance `infer_mobilenet_v2.py` (known-good `input.npy`) | 3 | 3 / 0 |
| Benchmark `bench_mobilenet_v2.py` (23 more inferences) | 23 | 23 / 0 |
...
| 6 | `CONTEXT_BINARY_GEN = NO` (QNN-5 skip) | `.so` direct inference path on x86 Linux (context binary optional on Linux) — acceptance passed |

---

## 6. User Prompts & Interventions

**Total user interventions: 8** (7 prompts + 1 interactive decision).
